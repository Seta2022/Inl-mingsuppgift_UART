/*
Denna kod är en del av ett program som hanterar LED-lampor. 
om Inkluderar LED_H så definerar LED_H dvs definerar header-filen.

Dom här två raderna nedanför  förhindrar att header-filen inkluderas flera gånger.
...
class Led{
  ...
  private:
      LedColor_Type color; // Atttribut för att hålla reda  på LED-färgen
      LedState_Type state; // Attribut för att hålla reda på LED-status

  public:
      // konstruktorn till klassen LED
      Led(LedColor_Type _color,LedState_Type _state);
      void setState(LedState_Type _state); //metod funktion för att ställa LED-status 
      LedState_Type getState() const; // Metoden för att hämta LED-status
